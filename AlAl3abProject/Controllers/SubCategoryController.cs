...
namespace AlAl3abProject.Controllers
{
    public class SubCategoryController : Controller
    {
        ...
        public async Task<IActionResult> IndexAsync(string id , string idd)
        {
            ...
            oIndexHomePageModel.lstItemSubCategory = _alAl3abDbContext.TbItemSubCategories.Where(a => a.SubCategoryId == Guid.Parse(id)).ToList();
            ...
            {
                foreach (var element in oIndexHomePageModel.lstItemSubCategory)
                {
            ...
            ViewBag.type = oIndexHomePageModel.lstMainCategories.Where(a => a.MainCategoryId == Guid.Parse(idd)).FirstOrDefault().TypeId;
            ViewBag.subcategoryid = id;
            return View(oIndexHomePageModel);
        }


        public async Task<IActionResult> IndexFilterAsync(string selectedValue2, string typeid , string subcategoryid)
        {
            IndexHomePageModel oIndexHomePageModel = new IndexHomePageModel();
            oIndexHomePageModel.LstCountries = countryService.getAll();
            oIndexHomePageModel.lstTypes = _alAl3abDbContext.TbTypes.ToList();
            ...
            List<TbItemSubCategory> tbItemSubCategories = new List<TbItemSubCategory>();
            List<TbItemSeller> tbItemSellers = new List<TbItemSeller>();
            oIndexHomePageModel.lstItemSubCategory = _alAl3abDbContext.TbItemSubCategories.Where(a => a.SubCategoryId == Guid.Parse(subcategoryid)).ToList();
